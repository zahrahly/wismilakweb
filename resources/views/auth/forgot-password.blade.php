...
<title>Forgot Password — Wismilak Premium Cigars</title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .form-subtitle { font-size: 1rem; color: rgba(245,235,224,.4); font-weight: 300; line-height: 1.6; }
  ...
  .back-to-login {
    ...
    color: var(--gold); text-decoration: none; font-size: .95rem; font-weight: 500;
  }
...
<div class="left-panel">
  ...
  <div class="left-content">
    <p class="brand-tag">Security & Heritage</p>
    <h1 class="brand-headline">
      Restore Your<br>
      <em>Access</em>
    </h1>
    <p class="brand-desc">
      Every key has a story. Reclaim yours and re-enter the world of Wismilak Premium Cigars.
    </p>
  </div>
</div>
...
<div class="right-panel">
  <div class="form-container">
    <div class="form-header">
      <p class="form-tag">Password Recovery</p>
      <h2 class="form-title">Forgotten?</h2>
      <p class="form-subtitle">No problem. Enter your email and we'll send a reset link to your humidor.</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.email') }}">
      ...
      <div class="input-wrapper">
        <input id="email" name="email" type="email" value="{{ old('email') }}" required autofocus
               class="form-input {{ $errors->has('email') ? 'is-error' : '' }}" placeholder=" ">
        <label class="input-label" for="email">Email Address</label>
        @error('email')<p style="color:#EF4444;font-size:.75rem;margin-top:.5rem;">{{ $message }}</p>@enderror
      </div>
